...
Scene::Scene()
{
	root = std::make_shared<SceneObject>(std::make_shared<GameObject>(GameObject().name = "root"));
	name = "Null";
}
...
std::shared_ptr<SceneObject> Scene::CreateGameObject()
{
	...
	auto output = std::make_shared<SceneObject>(std::make_shared<GameObject>(gameObject));
	output->parent = root;
...
{
	return RecursivelyGetAllChildren(scene.root);
}

std::vector<SceneObject> Scene::RecursivelyGetAllChildren(std::shared_ptr<SceneObject> object)
{
	std::vector<SceneObject> output;
	...
	for (auto child : object->children)
	{
		...
		auto childChildren = RecursivelyGetAllChildren(child);
