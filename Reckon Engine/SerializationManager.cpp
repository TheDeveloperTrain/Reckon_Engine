...
std::string SerializeTransformComponent(std::shared_ptr<Transform> transform);
...
void SerializationManager::SerializeSceneObject(std::ofstream& outFile, std::shared_ptr<SceneObject> object)
{
	...
	outFile << "gameobject:\n";
	outFile << "    transform: \n" << (SerializeTransformComponent(std::make_shared<Transform>(*object->object->transform)));
	outFile << "--\n";
...
std::string SerializeTransformComponent(std::shared_ptr<Transform> transform)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			output.append(std::to_string(transform->rotation.GetMatrix()[i][j]) + " ");
		}
	}
	output.append("\n");
